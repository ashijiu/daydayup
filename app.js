...
function updateInputFeedback() {
    ...
    // 背单词模式显示输入框
    elements.inputFeedback.style.display = 'flex';
    
    const currentWord = getCurrentWord();
    elements.inputFeedback.innerHTML = '';
    ...
    for (let i = 0; i < currentWord.length; i++) {
        ...
        if (i < appState.userInput.length) {
            ...
            letterDiv.classList.add(appState.isInputCorrect[i] ? 'correct' : 'incorrect');
        } else {
            letterDiv.textContent = '_';
            letterDiv.classList.add('empty');
        }
        
        elements.inputFeedback.appendChild(letterDiv);
    }
}
